...
from neuromirror.experiments.base import ExperimentPreset
from neuromirror.experiments.resting_state import RESTING_STATE_EYES_OPEN_CLOSED
...
def replay_frames(
    ...
    labels: list[str],
    config: ReplayConfig,
    experiment_preset: ExperimentPreset = RESTING_STATE_EYES_OPEN_CLOSED,
    ...
    experiment_payloads = experiment_preset.build_payloads(frames)
    for frame in frames:
        ...
        if "experiment" in experiment_payloads:
            frame["experiment"] = experiment_payloads["experiment"]
        if frame["frame_id"] == 0 and "spectrogram" in experiment_payloads:
            frame["spectrogram"] = experiment_payloads["spectrogram"]
        frame["scaling"] = {
            "band_power_unit": "V^2",
